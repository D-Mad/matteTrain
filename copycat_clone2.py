...
# ==========================================
# PHẦN 3: TILED INFERENCE & PREVIEW (ĐÃ ĐỒNG BỘ)
# ==========================================
def predict_tiled(model, image_path, tile_size=768, overlap=0.25, device='cuda'):
    """
    Hàm Inference thông minh: Tự động xử lý mọi kích thước ảnh (nhỏ hay to hơn tile_size)
    """
    model.eval()
    ...
    with torch.no_grad():
        for y in range(0, H, stride):
            for x in range(0, W, stride):
                ...
                # Cố gắng lấy đủ tile_size
                y2 = min(y + tile_size, H)
                x2 = min(x + tile_size, W)
                
                # Nếu ảnh lớn, mà crop bị hụt ở cuối -> lùi lại để lấy đủ tile đẹp
                ...
                # --- Xử lý kích thước động ---
                # Lấy size thực tế của tile vừa chạy (đề phòng ảnh nhỏ hơn tile_size)
...
def generate_preview(model, img_dir, save_dir, epoch, device, tile_size):
    """
    Tạo ảnh preview sử dụng CHÍNH XÁC thuật toán của Inference
    """
    valid_ext = ('.png', '.jpg', '.jpeg')
    files = sorted([f for f in os.listdir(img_dir) if f.lower().endswith(valid_ext)])
    if not files: return
    
    # Lấy ảnh đầu tiên làm mẫu
    test_img_path = os.path.join(img_dir, files[0])
    
    # Gọi hàm predict_tiled với tham số tile_size từ user
...
def train_mode(args):
    ...
    print(f"Preview Tile Size: {args.tile_size}") # Log ra để kiểm tra
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Generate Preview (Giờ dùng đúng tile_size)
...
def inference_mode(args):
    ...
    for f in tqdm(files, desc="Processing"):
        # Chạy Inference với tile_size user chọn
        res = predict_tiled(model, os.path.join(args.input, f), tile_size=args.tile_size, overlap=0.25, device=device)
...
if __name__ == '__main__':
    ...
    # Thêm tile_size vào train để preview chính xác
    p_train.add_argument('--tile_size', type=int, default=768, help='Kích thước tile dùng cho Preview (nên bằng Inference)')
