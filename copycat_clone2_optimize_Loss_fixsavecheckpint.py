...
def train_mode(args):
    ...
    # Tạo folder chứa checkpoints và previews
    checkpoint_dir = os.path.join(base_dir, "checkpoints")
    preview_dir = os.path.join(base_dir, "previews")
    os.makedirs(checkpoint_dir, exist_ok=True)
    os.makedirs(preview_dir, exist_ok=True)

    # Phân tích tên model để tạo versioning (ví dụ: mymodel.pth -> mymodel, .pth)
    model_name_base = os.path.basename(args.model_save)
    model_name_root, model_name_ext = os.path.splitext(model_name_base)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.save_freq == 0:
            # 1. Lưu file chính (Overwrite) -> Để dùng cho Resume lần sau dễ dàng
            torch.save(model.state_dict(), args.model_save)
            
            # 2. Lưu file Checkpoint có tên riêng (VD: mymodel_ep010.pth)
            new_ckpt_name = f"{model_name_root}_ep{epoch:03d}{model_name_ext}"
            new_ckpt_path = os.path.join(checkpoint_dir, new_ckpt_name)
